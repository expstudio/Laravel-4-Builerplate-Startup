<?php namespace Expstudio\LaraClip\File\Image;
...
class Resizer
{
	...
	public function resize($file, $style)
	{
		...
		$filePath = strpos($filePath, '?') !== FALSE ? substr($filePath, 0, strpos($filePath, '?')) : $filePath;
	...
	protected function parseStyleDimensions($style)
  	{
		...
		if (strpos($style->value, 'x') === false)
		{
			...
			return  array($width, null, 'landscape');
		}
		...
		if (empty($width))
    	{
			...
			return  array(null, $height, 'portrait');
		}
		...
		if ($resizingOption == '!')
		{
			...
			return  array($width, $height, 'exact');
		}

		// Let the script decide the best way to resize.
		return  array($width, $height, 'auto');
	}
	...
	{
		return call_user_func_array($callable,  array($file, $this->imagine));
	}
